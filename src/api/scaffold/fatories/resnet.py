...
class ResNet(ModelFactory):
    ...
    def _build_env(self):
        ...
        self._dir_dataset_train = os.path.join(self._dir_dataset, self.FLAG_TRAIN)
        self._dir_dataset_val = os.path.join(self._dir_dataset, self.FLAG_VAL)
        self._dir_dataset_test = os.path.join(self._dir_dataset, self.FLAG_TEST)
        for hook in [self._dir_dataset_train, self._dir_dataset_val, self._dir_dataset_test]:
            shutil.rmtree(hook, ignore_errors=True)
        for hook in [self._dir_dataset_train, self._dir_dataset_val, self._dir_dataset_test]:
            os.makedirs(os.path.join(hook, self.FLAG_POSITIVE_DIGIT), exist_ok=True)
            os.makedirs(os.path.join(hook, self.FLAG_NEGATIVE_DIGIT), exist_ok=True)

        # Check && Split Dataset
        _dir_dataset_yes = os.path.join(self._dir_dataset, "origin", self.FLAG_POSITIVE)
        _dir_dataset_bad = os.path.join(self._dir_dataset, "origin", self.FLAG_NEGATIVE)
        for hook, pnd in {
            _dir_dataset_yes: self.FLAG_POSITIVE_DIGIT,
            _dir_dataset_bad: self.FLAG_NEGATIVE_DIGIT,
        }.items():
            ...
            for fn in os.listdir(hook):
                ...
                if operator < self.RATIO_TRAIN:
                    shutil.copy(src_path_img, os.path.join(self._dir_dataset_train, pnd))
                elif operator < self.RATIO_TRAIN + self.RATIO_VAL:
                    shutil.copy(src_path_img, os.path.join(self._dir_dataset_val, pnd))
                else:
                    shutil.copy(src_path_img, os.path.join(self._dir_dataset_test, pnd))

    @staticmethod
    def _get_image_folder(dir_dataset: str, with_augment: bool) -> ImageFolder:
        ...
        return ImageFolder(root=dir_dataset, transform=transform_)
        ...
        for epoch in range(epochs):
            ...
            logger.debug(
                ToolBox.runtime_report(
                    ...
                    device=self.DEVICE,
                    epoch=f"[{epoch + 1}/{epochs}]",
                    avg_loss=f"{total_loss / len(data):.4f}",
                    avg_acc=f"{total_acc / len(data):.4f}",
                )
            )
    ...
    def _val(self, model: nn.modules, data_loader: DataLoader, data: ImageFolder):
        total_acc = 0
        for i, (img, label) in enumerate(data_loader):
            img = img.to(self.DEVICE)
            label = label.to(self.DEVICE)
        ...
        logger.success(
            ToolBox.runtime_report(
                ...
                task=self._task_name,
                size=len(data),
                total_acc=f"{total_acc / len(data):.4f}",
            )
        )
    ...
    def val(self, path_model: typing.Optional[str] = None):
        ...
        data = self._get_image_folder(self._dir_dataset_val, with_augment=False)
        data_loader = DataLoader(data, batch_size=self.BATCH_SIZE, shuffle=False)

        self._val(model, data_loader=data_loader, data=data)
    ...
    def train(self):
        ...
        data = self._get_image_folder(self._dir_dataset_train, with_augment=True)
